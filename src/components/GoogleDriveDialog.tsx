...
// "성과평가 결과" 화면 상단의 Excel 다운로드 옆에 두는 단 하나의 Google
// Drive 진입점. 팝오버 안에서 연결/저장/불러오기/파일보기를 모두 처리해,
// 화면에 구글 관련 버튼이 여러 개로 늘어나지 않게 한다.
export default function GoogleDriveDialog({ workspace, state, dispatch, buildReportWorkbook, buildSheetWorkbook }: GoogleDriveDialogProps) {
  ...
  return (
    <div className="relative shrink-0">
      <Button variant="secondary" onClick={openDialog} className="flex items-center gap-1.5 px-3 py-1.5">
        <DriveIcon className="h-4 w-4" /> Google Drive
      </Button>

      {open && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-4 pt-20" onClick={() => setOpen(false)}>
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-black">Google Drive</h3>
              <button onClick={() => setOpen(false)} className="text-gray-400 hover:text-black" aria-label="닫기">
                ✕
              </button>
            </div>
