// 피어리뷰 › 과제별. 평가과제마다 참여자 전원(본인 포함)을 순위로 평가하고 근거를 적는다.
// 기여도는 팀원에게 받지 않고 팀장이 평가하기에서 정한다. 예전에 받은 기여도 리뷰는
...
export default function TaskPeerPanel() {
  ...
  const legacyContribution = state.taskPeerReviews.filter((r) => r.method === 'contribution')
                ...
                <p key={tid} className="text-xs text-gray-600">
                  ...
                  {targets
                    .map((id) => {
                      ...
                      const avg = mine.reduce((a, r) => a + r.value, 0) / mine.length
                      return `${state.members.find((m) => m.id === id)?.name ?? '?'} ${avg.toFixed(0)}%`
                    })
                    .join(' · ')}
                </p>
              )
