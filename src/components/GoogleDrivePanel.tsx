        ...
        {saveChoice ? (
          <div className="mt-2 space-y-1.5 rounded-card border border-separator p-2.5">
            ...
            <div className="flex gap-1.5">
              <Button variant="primary" onClick={() => void runSave('update')} size="sm" disabled={busy !== null} className="flex-1">
                업데이트
              </Button>
              <Button variant="secondary" onClick={() => void runSave('new-version')} size="sm" disabled={busy !== null} className="flex-1">
                새 버전으로 저장
              </Button>
            </div>
            ...
            {restoreList.map((item) => (
              <li key={item.fileId} className="rounded-card border border-separator px-2.5 py-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Button variant="secondary" onClick={() => setRestoreTarget(item)} size="sm" disabled={busy !== null}>
                    불러오기
                  </Button>
                </div>
                  ...
                  <div className="mt-2 space-y-1.5 rounded-card bg-accent-soft p-2.5 text-[13px] text-label">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      <Button variant="primary" onClick={() => void doRestore(item, true)} size="sm" disabled={busy !== null}>
                        {busy === 'restoring' && <Spinner className="h-3.5 w-3.5" />}지금 데이터 백업 후 교체
                      </Button>
                      <Button variant="danger" onClick={() => void doRestore(item, false)} size="sm" disabled={busy !== null}>
                        바로 교체
                      </Button>
                      <button onClick={() => setRestoreTarget(null)} className="text-label-2 hover:text-label">
                        취소
                      </button>
                    </div>
                  </div>
