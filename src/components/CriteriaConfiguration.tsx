...
export default function CriteriaConfiguration() {
  ...
  const peerReviewDescription =
    rw === 0
      ? '사용 안 함 — 팀원면담 탭에서 피어리뷰를 업로드해도 점수 계산에는 영향을 주지 않습니다.'
      : `사용 시 팀원면담 탭에서 업로드한 피어리뷰 등급의 평균 배율이 1.0 기준에서 섞입니다. 지금 비율 기준: 평균 등급 S(100점)면 ${fmt(blendByWeight(1, 1.0, rw))}배, D(60점)면 ${fmt(blendByWeight(1, 0.6, rw))}배.`

  return (
    <div>
      <h2 className="text-xl font-bold text-black">기준 설정</h2>
      <p className="mt-1 text-sm text-gray-600">
        평가에 반영할 기준을 선택하세요. 켜면 반영 비율(0~100%)을 슬라이더로 세밀하게 조절할 수 있습니다. 변경 시
        평가 결과가 자동으로 재계산됩니다.
      </p>
