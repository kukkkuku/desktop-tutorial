...
// 구글시트 연결(빠른 시작) 화면을 그 시트 · 그 L1 이 골라진 채로 연다. 이 탭(sessionStorage)에만 둔다.
const KEY = 'perf-import-request'

export interface PerfImportRequest {
  url: string // 구글시트 링크(탭 gid 포함)
  l1: string // 고를 L1(그 아래 L2를 모두 골라 둔다)
}
...
export function takePerfImportRequest(): PerfImportRequest | null {
  try {
    ...
    return v && typeof v.url === 'string' && typeof v.l1 === 'string' ? v : null
  } catch {
    return null
  }
}
