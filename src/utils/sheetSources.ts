...
// ---------- 값(표 형태) 읽기·쓰기 ----------
// 피어리뷰 요청·응답처럼 앱이 쓰는 숨은 탭에 줄 단위로 적고 읽는다.

export async function readSheetValues(spreadsheetId: string, tab: string, cols = 'A:Z'): Promise<string[][]> {
  const range = encodeURIComponent(`${quoteTab(tab)}!${cols}`)
  const res = await sheetsFetch<{ values?: unknown[][] }>(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range}?valueRenderOption=FORMATTED_VALUE`,
  )
  return (res.values ?? []).map((r) => r.map((v) => (v === null || v === undefined ? '' : String(v))))
}

export async function appendSheetValues(spreadsheetId: string, tab: string, rows: (string | number)[][]): Promise<void> {
...
// cell: 'E3'처럼 칸 하나
export async function updateSheetValue(spreadsheetId: string, tab: string, cell: string, value: string): Promise<void> {
  const range = encodeURIComponent(`${quoteTab(tab)}!${cell}`)
  await sheetsFetch(
...
// 숨은 탭을 만든다(이미 있으면 그대로). 첫 줄에 머리글을 적는다.
export async function ensureHiddenTab(spreadsheetId: string, tab: string, header: string[]): Promise<void> {
  const { tabs } = await fetchSpreadsheetTabs(spreadsheetId)
  if (tabs.some((t) => t.title === tab)) return
  await sheetBatchUpdate(spreadsheetId, [
    { addSheet: { properties: { title: tab, hidden: true, gridProperties: { rowCount: 200, columnCount: header.length + 2 } } } },
  ])
  await appendSheetValues(spreadsheetId, tab, [header])
}
